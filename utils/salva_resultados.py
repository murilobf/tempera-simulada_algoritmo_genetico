...
def salva_resultado_ag(tag, cidades, matriz_dist, resultado):
    os.makedirs(DIR_SAIDA, exist_ok=True)
    ...
    metricas = {
        ...
        "config": {
            "tempo_maximo": resultado.config.tempo_maximo,
            "qtde_nos": resultado.config.qtde_nos,
            "max_populacao": resultado.config.max_populacao,
            "taxa_mutacao": resultado.config.taxa_mutacao,
            "seed": resultado.config.seed,
        },
    }
    caminho_json = os.path.join(DIR_SAIDA, f"{tag}_metricas.json")
    ...
    # grafico de convergencia (custo medio da populacao vs melhor custo)
    hist = resultado.historico
    fig, ax1 = plt.subplots(figsize=(8, 5))
    ax1.plot(hist.iteracao, hist.custo_medio, color="tab:blue", alpha=0.4, label="Custo médio da população")
    ax1.plot(hist.iteracao, hist.melhor_custo, color="tab:red", linewidth=2, label="Melhor custo")
    ax1.plot([],[], " ", label=f"Tempo de execução: {resultado.tempo_execucao_s:.2f}s")
    ax1.set_xlabel("Geração")
    ax1.set_ylabel("Custo da rota")
    ax1.set_title(f"Convergência do Algoritmo Genético ({tag})")
    ax1.legend(loc="upper right")
    fig.tight_layout()
    fig.savefig(os.path.join(DIR_SAIDA, f"{tag}_convergencia.png"), dpi=150)
    plt.close(fig)
